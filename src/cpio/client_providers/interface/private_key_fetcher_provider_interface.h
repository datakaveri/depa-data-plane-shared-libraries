...
struct PrivateKeyFetchingRequest {
  ...
  // Raw X25519 private key bytes (32 bytes) of the ephemeral key pair
  // generated for this request. The corresponding public key is sent as the
  // nonce in the OIDC token so the vending service can encrypt the response
  // key material for this TEE instance. Used by the GCP TEE path only.
  std::string ephemeral_private_key_bytes;
};
