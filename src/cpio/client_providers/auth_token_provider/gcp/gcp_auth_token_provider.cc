...
constexpr std::string_view kNonceParameter = "nonce=";
...
ExecutionResult GcpAuthTokenProvider::GetSessionTokenForTargetAudience(
  ...
  std::string query =
      absl::StrCat(kAudienceParameter,
                   *get_token_context.request->token_target_audience_uri, "&",
                   kFormatFullParameter);
  if (!get_token_context.request->nonce.empty()) {
    absl::StrAppend(&query, "&", kNonceParameter,
                    get_token_context.request->nonce);
  }
  http_context.request->query = std::make_shared<std::string>(std::move(query));
