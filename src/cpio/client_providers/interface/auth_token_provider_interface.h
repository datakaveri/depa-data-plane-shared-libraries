...
#ifndef CPIO_CLIENT_PROVIDERS_INTERFACE_AUTH_TOKEN_PROVIDER_INTERFACE_H_
#define CPIO_CLIENT_PROVIDERS_INTERFACE_AUTH_TOKEN_PROVIDER_INTERFACE_H_
...
struct GetSessionTokenForTargetAudienceRequest {
  ...
  // Optional nonce to embed in the issued OIDC token (e.g. base64-encoded
  // TEE ephemeral public key). When non-empty, appended as the `nonce`
  // query parameter so the vending service can verify the token was issued
  // for this specific TEE instance.
  std::string nonce;
};
...
#endif  // CPIO_CLIENT_PROVIDERS_INTERFACE_AUTH_TOKEN_PROVIDER_INTERFACE_H_
